...
function SearchBar({onSearch, currentSearch}){
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if(form.search.trim().length > 1){
            onSearch(form.search)
        }
    }
    return (
        <>
            <form className="flex flex-col sm:flex-row justify-center items-center m-5 sm:mx-10 p-2 rounded-4xl gap-5 lg:mx-[10%] xl:mx-[15%] 2xl:mx-[20%] sm:shadow sm:bg-light" onSubmit={handleSubmit}>
                <label className="flex shadow sm:shadow-none p-3 w-full rounded-4xl gap-2 sm:gap:5 bg-light">
                    ...
                    <input type="text" placeholder="Search by title, author or ISBN..." className="w-full outline-none " name="search" value={form.search} onChange={handleChange}/>
                </label>
                <button className="w-full bg-principal p-3 rounded-4xl text-background font-bold sm:w-50 cursor-pointer hover:bg-principal/85"  type="submit">Search <span className="sm:hidden">Library</span></button>
            </form>
